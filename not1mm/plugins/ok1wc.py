"""OK1WC Memorial Contest (MWC) plugin"""

# pylint: disable=invalid-name, c-extension-no-member, unused-import

# 1 hour every Monday 16:30 UTC, CW-only, 40m/80m
# serial exchange
# one point per QSO
# multiplier is last character of call, once per band
# Cabrillo name: OK1WC
...
def set_contact_vars(self):
    ...
    result = self.database.fetch_band_lastletter_exists(
        self.contact.get("Band", ""), self.contact.get("Call", "")
    )
    ...
    logger.info(f"contact {self.contact}")
...
def show_mults(self):
    ...
    result = self.database.fetch_band_lastletter_count()
    if result:
...
def adif(self):
    ...
    gen_adif(self, cabrillo_name, "OK1WC")
...
def recalculate_mults(self):
    ...
    for contact in all_contacts:
        ...
        result = self.database.fetch_band_lastletter_exists_before_me(
            self.contact.get("Band", ""), self.contact.get("Call", ""), time_stamp
        )
...
def get_mults(self):
    ...
    mults = {}
    mults["wpxprefix"] = show_mults(self)  # online shown as Suffix Letter
    return mults
