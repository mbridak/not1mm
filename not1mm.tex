...
Of note, \index{State QSO parties}State QSO parties. A PR has been submitted adding minor support for QSO parties, Mainly logging and Cabrillo\index{Cabrillo} generation, Not scoring or mult tracking.
\newpage
...
\subsubsection{The single line install}
\begin{verbatim}
    •curl -LsSf uvx.sh/not1mm/install.sh | sh
\end{verbatim}
\subsubsection{The only slightly more involved way}
...
To setup \index{CAT}CAT control, \index{CW keyer}CW keyer, and \index{callsign lookups}callsign lookups
...
Under the `Cluster`\index{Cluster} tab you can change the default AR\index{AR} Cluster server, port, and filter settings used for the bandmap window.
