...
\begin{itemize}
    ...
    \item REF CW, SSB
    \item RSGB 80M CC
    \item RSGB Commonwealth
    \item RSGB IOTA
    \item Russia DX
    \item Russian District Award
    \item SAC CW, SSB
    \item SPDX
    \item SSA MT
...
\subsection{Flatpak}
Not1MM can now be installed via flatpak. Look in the Github repos README.md for a link to download the not1mm.flatpak file.
\subsubsection{Flatpak Install}
Once downloaded you can install the flatpak with:
...
\end{verbatim}
\subsubsection{Multiple line install and updates}
Install uv as described at \url{https://docs.astral.sh/uv/}. Run the following line in the terminal:
\begin{verbatim}
    curl -LsSf https://astral.sh/uv/install.sh | sh
\end{verbatim}
Instruct it to run Not1MM without installing:
\begin{verbatim}
    uvx not1mm@latest
\end{verbatim}
Either install or update it with:
\begin{verbatim}
...
\noindent You can also rollback to an earlier version of Not1MM if the latest update has a bug (This happens a lot as we test in production). To run version 23.5.29 of the software with Python 3.11, for example, you would type:

\begin{verbatim}
    • uvx --python 3.11 not1mm==23.5.19
\end{verbatim}
...
\begin{tabular}{| c | p{9cm} |}
	...
	\hline
\end{tabular}
\subsubsection{And how to edit them}
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{pic/edit_keys.png}
\vspace{0.5cm}\\
Luckily now you don't have to be stuck with my preferred bindings. The \index{key bindings} key bindings can now be edited \textbf{File >> Edit Keys}.
...
\subsection{The Bandmap and Cluster Windows}

\textbf{Window >> Bandmap} and \textbf{Window >> Cluster} respectively.

The Cluster window is used to connect to the cluster node specified in the Configuration Settings. Press the Connect button in the cluster window to start the inflow of spots. The Bandmap window displays worked stations and spots that have been received by the Cluster window, sorted by frequency.  When CAT\index{CAT} is enabled, the bandmap\index{bandmap} will track with the VFO\index{VFO}. This is still a work in progress.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{pic/bandmap.png}
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{pic/cluster_spots.png}
\vspace{0.5cm}

The VFO\index{VFO} frequency is indicated with a small green triangle above the tick marks. A vertical blue bar displays the receive filter bandwidth\index{bandwidth} reported by the radio if available via CAT\index{CAT}.
...
\section{ESM}
\index{ESM}
To turn on ESM or Enter Sends Message you can go to \textbf{FILE >> Configuration Settings}
