...
\vspace{0.5cm}
Not1MM's interface is a blatant rip-off of \index{N1MM}N1MM. It is NOT N1MM and any problem you have with this software should in no way reflect on their software.
...
\subsection{The Why}
\textbf{Currently, this exists for my own personal enjoyment}. I recently retired after 35+ years working for 'The Phone Company', GTE → Verizon → Frontier. And being a Gentleman of Leisure, I needed something to do in my free time. I am a casual contester and could not find any contesting software for Linux that I wanted to use. \textbf{Tucnak} is very robust and mature contest logger that is available at \url{http://tucnak.nagano.cz/}. It just wasn't for me.
\subsection{Target Environment}
The target operating system for this application is \index{Linux}Linux. It may be able to run on other platforms such as \index{BSD}BSD and \index{Windows}Windows. But I don't have a way, or desire, to directly support them. I've recently purchased an M4 \index{Mac}Mac Mini, so I'll probably put more effort into that platform as well.
...
\begin{itemize}
    \item \index{HamLib}HamLib before 4.6.3 had a problem with sending CW and changing/reading the keying speed.
    \item wfview before version 2.2 has issues with frequency reporting and CW sending.
\end{itemize}
\newpage
...
At present, there are no specific \index{State QSO parties}State QSO parties. There are, however, plug-ins for generic QSO parties that use either a signal report or serial number for the exchange.
\newpage
\section{Installation}

\subsection{Not1MM Dependencies}
...
\subsubsection{Single line install}
From the terminal type:
\begin{verbatim}
    • curl -LsSf uvx.sh/not1mm/install.sh | sh
\end{verbatim}
\subsubsection{Multiple line install}
Install uv as described at \url{https://docs.astral.sh/uv/}. Run the following line in the terminal:
\begin{verbatim}
...
\noindent That's it... it will go out, fetch the latest version of Not1MM, setup a Python virtual environment, get all the needed python libraries, cache everything, and run Not1MM. 
\subsubsection{Running the software}
Start Not1MM from the command line by typing: 
\begin{verbatim}
    • not1mm
\end{verbatim}
The first time it runs will take a minute, but on subsequent launches it's lightning quick and will automatically check for updates. You will prompted to download the latest version. A lovely non-AI generated icon may appear on the desktop that can be clicked to launch the application.
\subsubsection{Python versions and rollbacks}
...
\noindent You can also rollback to an earlier version of Not1MM if the latest update has a bug (This happens a lot as we test in production).To run version 23.5.29 of the software with Python 3.10, for example, you would type:
...
\section{Possible Post-Install Issues }

\subsection{Warning Message 1}
...
\noindent Try logging out and logging back in. Or reboot.

\subsection{Warning Message 2}
...
\noindent Use your package manager to load \textbf{libxcb-cursor0}.\\
...
\noindent For a more permanent solution, you can place the line:

\begin{verbatim}
export QT_QPA_PLATFORM=wayland
\end{verbatim}

\noindent in the .bashrc file of your home directory. Then after logging out and back in you should be able to launch it normally.

\section{Various File Locations}

\subsection{Data Files}

If your system has an XDG\_DATA\_HOME environment variable set, the database and CW macro files can be found there. Otherwise they will be in $\sim$/.local/share/not1mm

\subsection{Configuration Files}

Configuration file(s) can be found at the location defined by
\begin{verbatim}XDG_CONFIG_HOME\end{verbatim}
Otherwise they will be found in the home directory at \begin{verbatim}~/.config/not1mm\end{verbatim}
\section{The Database}

\subsection{Why}

The \index{database}database holds... wait for it... data... I know, shocker right? A database can hold one or many contest logs. It also holds the station information, everything shown in the Station Settings dialog. You can have one database for the rest of your life. Filled with hundreds of contests you've logged. Or, you can create a new database to hold just one contest. You do you Boo.
...
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/settings.png}
\vspace{0.5cm}
...
Before operating in a contest, you might want to update the \index{CTY}CTY and \index{SCP}SCP files. You can do this by choosing \textbf{FILE >> Update CTY} and \textbf{FILE >> Update MASTER.SCP}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/new_contest.png}
\vspace{0.5cm}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/select_contest.png}
\vspace{0.5cm}
...
To setup \index{CAT}CAT control, \index{CW keyer}CW keyer, \index{callsign lookups}callsign lookups, and \index{cluster} telnet cluster
select \textbf{File >> Configuration Settings}.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/configuration_settings.png}
\vspace{0.5cm}
...
Under the CAT\index{CAT} tab, you can choose one of the following:

\begin{itemize}

 \item rigctld\index{rigctld} normally with an IP of 127.0.0.1 and a port of 4532

 \item flrig\index{FLRig} normally with an IP of 127.0.0.1 and a port of 12345
...
\begin{itemize}
 \item cwdaemon\index{cwdaemon} normally uses IP 127.0.0.1 and port 6789
...
\begin{itemize}
 \item Speed Increment for PgUp/PgDown: The PgUp and PgDown keys increment the keyer speed by the specified amount. The default step is $\pm$ 1 wpm with option to select steps anywhere in the range 1--10 wpm.
 
 \item Send 599 RST as: If a CW contest requires an RST report, the convention is to always use 599. This can be sent as either 5NN, 599, or ENN. 5NN is the most common.

 \item Serial Number Padding Character: Contest serial numbers can be optionally zero-padded. Choices for the padding character are 0, O, or T.

 \item Padding Length: Pad serial numbers with a specified number of padding characters. As an example, selecting the character T with a padding length of 3 sends the serial number 8 as TT8. Setting the padding length to 1 will result in no padding.

 \item Specify how to send characters 1, 9, 0 in contest serial numbers. When sent as cut numbers, the character A replaces 1, N replaces 9, and T replaces 0.
\end{itemize}

\subsection{Cluster}

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/configuration_cluster.png}
\vspace{0.5cm}

\noindent Under the Cluster\index{Cluster} tab you can change the default AR\index{AR} cluster server, port, and filter settings used for the bandmap window. Setting the Mode to OPEN will display all spots on the selected band originating from all spotters anywhere in the world. Most users will want to enable a FILTER for spotters in the region in which they are located. The filter can be highly customized to select continents, countries, CQ zones, specific callsigns, and even Boolean filter combinations. To select spotting stations located in Europe, for example, the filter text can be set as:
\begin{verbatim}
set/dx/filter spottercont=eu
\end{verbatim}
Detailed information can be found at \url{http://www.nc7j.com/arcluster/arusermanual-6/AR-Cluster\%20DX.html}.

\noindent Older versions of Python may not be able to handle the data rate of the telnet cluster stream, particularly during busy contests. This will cause spots on the bandmap to lose synchronization with the cluster. To remedy this, Not1MM can be run with the current version of Python inside a virtual environment as described in \ref{subsec:rollback}.
...
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/n1mm_packet_config.png}
\vspace{0.5cm}
...
\subsection{Bands}
You can define which bands\index{bands} appear in the main window. Those with checkmarks will appear. Those without will not.

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/configure_bands.png}
\vspace{0.5cm}

\subsection{Options}
A variety of features can be enabled in the Options tab:
\begin{itemize}
\item Use Enter Sends Message (ESM)\index{ESM} and configure its function keys.
\item Allow the program to access Call History\index{Call History} info.
\item Periodically send XML score to an online scoreboard.
\item Clear the input fields in S\&P mode when the VFO\index{VFO} frequency changes.
\end{itemize}

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/configuration_options.png}
\vspace{0.5cm}

Three real-time scoreboards\index{real-time scoreboards} are supported:
\begin{itemize}
...
Not1MM listens for WSJT-X\index{WSJT-X} UDP\index{UDP} traffic on the Multicast\index{Multicast} address 224.0.0.1:2237. This should work by default with Not1MM. That's good because I'm lazy.\\Not1MM watches for fldigi\index{fldigi} QSOs by monitoring UDP traffic on 127.0.0.1:9876.

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/fldigi_adif_udp.png}
\vspace{0.5cm}

The F1--F12 function keys are sent to fldigi\index{fldigi} via XMLRPC\index{XMLRPC}. Fldigi is placed into TX mode, the message is sent, and a $\wedge$r is appended to place it back into RX mode.

Unlike WSJT\index{WSJT}, fldigi\index{fldigi} needs configuration of some settings. The XMLRPC interface needs to be active. In fldigi's config dialog, go to \textbf{CONTESTS >> General >> CONTEST} and select Generic Contest. Make sure the Text Capture Order field says CALL EXCHANGE.
\newpage
\section{Operating Multi-Multi}

Work is underway on Multi-Multi\index{Multi-Multi} contest operations. There is a companion project \textbf{renfield}\index{renfield}\\
\emph{https://github.com/mbridak/renfield} that will be needed for this. The idea is to have a separate slave server running, preferably on another computer that's on the same network as the contesting PCs. This server will handle all the database CRUD operations. It will assign serial numbers, check for duplivate contacts, and generate a Cabrillo\index{Cabrillo} file.

\vspace{0.5cm}
\begin{center}
\includegraphics[width=0.75\linewidth]{../pictures/renfield_cli.png}
\end{center}
\vspace{0.5cm}
...
\vspace{0.5cm}
\begin{center}
\includegraphics[width=0.5\linewidth]{../pictures/configuration_group.png}
\end{center}
\vspace{0.5cm}

One computer needs to be the master station. The master station will tell the renfield\index{renfield} server what contest is being run.

\subsection{Contest Settings for Multi-Multi}
In Contest Settings, if the Operator is set to "MULTI-OP"\index{MULTI-OP} and the Transmitter category is not "ONE" or "SWL", Not1MM will ask the renfield\index{renfield} server for serial numbers\index{serial numbers} and dupe checks.

\vspace{0.5cm}
\begin{center}
\includegraphics[width=0.5\linewidth]{../pictures/multi_multi.png}
\end{center}
\vspace{0.5cm}
...
To edit the macro\index{macro} keys, choose \textbf{File >> Edit Macros}. This will open the system default text editor and display the current active macros.   The current operating mode (CW, SSB or RTTY) and contest will automatically select the corresponding macro file. Each contest and mode has its own unique copy of the macros -- these are not global settings. When finished editing, save the file and close the editor.
...
\begin{tabular}{| c | p{9cm} |}
    ...
    \# & Sends serial number. \\
    \hline
    \{VOICE1\} - \{VOICE10\} & Uses `rigctld`\index{rigctld} to send voice macros stored in the radio. \\
    \hline
...
If you use rigctld\index{rigctld} and your radio supports it, you can use the macros\index{macros} {VOICE1}, {VOICE2}\index{VOICE}, etc to send the recorded audio messages stored in the radio.
\subsubsection{Voice macro .wav files}
The macros used with voice can also linked to .wav files\index{wav files}. Specify the filename, but exclude the .wav extension. The filename must be lowercase and enclosed in brackets. For example `[cq]` will play `cq.wav`, `[again]` will play `again.wav`. The .wav files are stored in the data directory.  See \textbf{Various data file locations} above for the location of these data files. For me, the macro `[cq]` will play 
\begin{verbatim}/home/mbridak/.local/share/not1mm/K6GTE/cq.wav
\end{verbatim}

\textbf{The default .wav files are not the ones you will want to use. They sound like an idiot.}\index{idiot} Use a program like Audacity\index{Audacity} or similar to record new .wav files in your own voice.
...
Macros can also be used to send CAT/CI-V control codes\index{CAT/CI-V} to the radio. To make use of this feature, start the command with the letters "RI" and a colon, followed by the desired command. If the command is ASCII text (for example, for Yaesu radios), just enter the text after "RI:". For binary\index{binary} codes, enter hexadecimal\index{hexadecimal} values separated by spaces.
...
Note that CAT/CI-V command macros are only available when either FLRig\index{FLRig} or rigctld\index{rigctld} (HamLib)\index{HamLib} are being used for transceiver control.

\newpage
\section{cty.dat and QRZ for Distance and Bearing}

When a callsign is entered, a lookup\index{lookup} is done in the cty.dat\index{cty.dat} file to identify the country of origin, geographic center, CQ\index{CQ} zone, and ITU\index{ITU} region. Great circle calculations are then done to determine the heading\index{heading} and distance\index{distance} from the home grid\index{grid} square to the geographic center of the DX station. This information is displayed at the bottom left.

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/heading_distance.png}
\vspace{0.5cm}

Next, a request is made to qrz.com\index{QRZ} for the grid square of the callsign. If there is a response, more accurate distance\index{distance} and bearing\index{bearing} numbers are displayed. The `Regional`\index{Regional} text is replaced with the station's specific 6-digit grid\index{grid} square as shown below.

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/heading_distance_qrz.png}
\vspace{0.5cm}

\section{Other Uses for the Callsign Field}
\textbf{The SPACE bar must be pressed after entering any of the following functions.}
\subsection{Frequency}
\textbf{Frequency}\index{Frequency} Enter a frequency in kHz. This changes the active logging band. If CAT\index{CAT} control is enabled, the specified frequency will change the transceiver VFO\index{VFO}.
\subsection{Modes}
\textbf{CW, SSB, RTTY}\index{CW}\index{SSB}\index{RTTY} Sets the mode being logged. If CAT\index{CAT} control is enabled, the mode\index{mode} changes on the radio.
\subsection{Change Operator}
\textbf{OPON}\index{OPON} Specify the operator currently logging.
 \\
 \\ 
...
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/mainwithcallouts.png}
\vspace{0.5cm}
...
\begin{tabular}{| c | p{9cm} |}
    ...
    \hline
    Arrow-Down & Jump to the next spot below the current VFO\index{VFO} cursor in the bandmap window (CAT\index{CAT} Required). \\
    \hline
    ...
    \hline
    ALT-V	& Toggle the VFO\index{VFO} window. \\
    \hline
...
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/logdisplay.png}
\vspace{0.5cm}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/edit_cell.png}
\vspace{0.5cm}

Double-click a cell in the log window to edit its contents. Right-clicking on a cell brings up the edit dialog.

\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/edit_dialog.png}
\vspace{0.5cm}

You cannot directly edit the multiplier\index{multiplier} status of a contact. See the next section on recalculating multipliers. When editing a logged callsign, it's a good idea to check if other data such as the WPX field are still valid.
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/bandmap.png}
\vspace{0.5cm}

The VFO\index{VFO} frequency is indicated with a small green triangle above the tick marks. A vertical blue bar displays the receive filter bandwidth\index{bandwidth} if available via CAT\index{CAT}.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/VFO_and_bandwidth_markers.png}
\vspace{0.5cm}

Clicking on a spot\index{spot} sets the VFO\index{VFO} and populates the callsign field. Previously worked callsigns are displayed in gray. Callsigns marked with CTRL-M are color highlighted. A small icon after the callsign indicates the mode as follows:\\
...
The icon is followed by the UTC\index{UTC} time of the spot\index{spot}. The screenshot below shows CW\index{CW} and FT8\index{FT8} spots on the 20m band.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/bandmap_icons.png}
\vspace{0.5cm}

Right-clicking on a spot opens a menu to manage the entry:

\begin{itemize}
    \item[] Confirm: Refresh spot expiration timestamp and delete all other spots on this frequency (+- 0.1 kHz).
    \item[] Mark: Mark/Unmark spot to work later and delete all other spots on this frequency.
    \item[] Delete: Remove spot.
\end{itemize}
...
As a callsign is entered, the Check Window\index{Check Window} displays potential matches from the MASTER.SCP\index{MASTER.SCP} file, the local log, and recent telnet cluster spots. The MASTER.SCP column shows candidates with 3 or more matching characters indexed from the start of the callsign string. The local log and telnet columns match character strings of any length. Clicking on any of these candidate callsigns will automatically populate the callsign field.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/checkwindow.png}
\newpage
\subsection{The Rate Window}

\textbf{Window >> Rate Window}

This window contains QSO \index{rates} rates and counts.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/rate_window.png}

\subsection{The DXCC window}
\textbf{Window >> DXCC}

This window displays a table of worked DXCC \index{DXCC} entities arranged by band. When optional automatic scrolling is enabled, entering a callsign will scroll to the corresponding DXCC entity prefix.

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/dxcc_window.png}

\subsection{The Zone window}
\textbf{Window >> Zone}

\index{Zone}
This window functions the same way as the DXCC \index{DXCC} window, but for CQ\index{CQ} or ITU\index{ITU} zones. When automatic scrolling is enabled, check the appropriate radio button to select CQ or ITU zones as determined by the DX contest requirements.

\subsection{The Rotator Window (Work In Progress)}
\textbf{Window >> Rotator}

The Rotator\index{Rotator} window is a work in progress. The Rotator window relies on the functionality of the rotctld \index{rotctld} daemon. It connects to rotctld on address 127.0.0.1 and port 4533. You can change this if needed in the configuration dialog under the rotator tab. If started and there is no connection, you will see this:

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/rot1.png}
\vspace{0.5cm}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/rot2.png}
\vspace{0.5cm}

Once a call is entered and the bearing to contact is calculated you will see a needle appear:

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/rot3.png}
\vspace{0.5cm}
...
When operating a remote station, the transceiver VFO\index{VFO} can be controlled using inexpensive hardware in tandem with the Remote VFO\index{VFO} window. A description of the hardware can be found at the following link:

\begin{verbatim}
    https://github.com/mbridak/not1mm/blob/master/usb\_vfo\_knob/vfo.md
\end{verbatim}

Communicate with it using the VFO\index{VFO} Window: \textbf{Window >> VFO}

\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/vfo.png}
\newpage
\section{Cabrillo}
\index{Cabrillo}
...
\index{Mults}
The program automatically updates the score as each new entry is logged. There may be a situation where a station has been logged incorrectly. This QSO can be easily edited as described above, but this may add or remove a multiplier. To remedy this and correct the score, a Recalculate Mults function is located in the Misc menu. The Cabrillo\index{Cabrillo}/ADIF\index{ADIF} files must be regenerated.
\newpage
\section{ESM}
\index{ESM}
...
\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_cq.png}

\subsubsection{Call Entered Send His Call and the Exchange}

\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_withcall.png}

\subsubsection{Empty Exchange Field Send AGN Till You Get It}

\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_empty_exchange.png}

\subsubsection{Exchange Field Filled, Send TU QRZ and Logs it}

\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_qrz.png}
...
\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_sp_call.png}

\subsubsection{If No Exchange Entered Send AGN}

\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_sp_agn.png}

\subsubsection{With Exchange Entered, Send Your Exchange and Log it}

\vspace{0.5cm}
\includegraphics[width=0.75\linewidth]{../pictures/esm_sp_logit.png}
\newpage
\section{Call History Files}
\index{Call History files}
...
\vspace{0.5cm}
\includegraphics[width=0.9\linewidth]{../pictures/call_history_example.png}
\vspace{0.5cm}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/ss_parser_1.png}
\vspace{0.5cm}
...
\vspace{0.5cm}
\includegraphics[width=0.5\linewidth]{../pictures/sent_exch_sn.png}
\vspace{0.5cm}
...
\subsection{DX-Pedition}

This requested plugin is ment for operating a dxpedition\index{DXPedition}. There's no scoring. No cabrillo generator. Be in Run mode with ESM. ESM mode opperates differently for this plug in than any other contest. The cursor does not advance to other fields when pressing enter.

\begin{itemize}
    \item [] Pressing enter with no call in the callsign field sends the CQ macro.
    \item [] Pressing enter with a call in the callsign field sends HISCALL and the EXCHange macro.
    \item [] Pressing enter again with an unaltered callsign in the field will send the QRZ macro and log the contact.
    \item [] Pressing enter again with a different call than the one that was used two steps ago will assume you have corrected the call and send HISCALL macro and QRZ macro and log the contact.
\end{itemize}
\printindex
\end{document}
